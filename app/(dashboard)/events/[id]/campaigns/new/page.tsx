...
export default function NewCampaignPage() {
  ...
  const handleEditorError = useCallback((error: Error) => {
    console.error('[NewCampaignPage] Editor error:', error)
  ...
  const handleSaveDesign = async () => {
    if (!editorRef.current || !isEditorReady || !campaignId) return false
    ...
    try {
      ...
      console.error('[NewCampaignPage] Save error:', error)
  ...
  const handleNext = async () => {
    if (currentStep === "details") {
      ...
      if (isEmail) {
        ...
        // WhatsApp or SMS design — just move to review
  ...
  const handleFinish = () => {
    router.push(`/events/${eventId}/campaigns`)
  }
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-shrink-0">
        <PageHeader 
          ...
          description={`Create ${isWhatsApp ? 'a WhatsApp' : isSms ? 'an SMS' : 'an email'} campaign to send to your event guests`}
        >
          <Link href={`/events/${eventId}/campaigns`}>
                ...
                <div className="grid grid-cols-3 gap-3">
                  {channelOptions.map((ch) => (
                    ...
                    >
                      ...
                      {ch.value === "sms" && formData.channel !== "sms" && null}
                    </button>
                  ))}
                </div>
          ...
          <div className="h-full overflow-auto p-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Review your campaign before saving</CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
      ...
      {/* Footer Navigation */}
      <div className="flex-shrink-0 border-t border-ora-sand px-6 py-4">
