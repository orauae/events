...
// Execution status configuration with Waiting status
...
/**
 * Check if a step is a wait/delay step based on its output.
 */
...
/**
 * Get wait information from a step's output.
 */
...
function CurrentNodeDisplay({ 
  ...
  currentNode: AutomationNode | null; 
  displayStatus: DisplayStatus 
}): React.ReactNode {
  if (!currentNode || (displayStatus !== "Running" && displayStatus !== "Waiting")) {
    return null
  }
  ...
  return (
    <div className="mt-4 p-3 rounded-lg bg-blue-50 border border-blue-200">
      ...
      <p className="text-sm text-blue-700 mt-1">
        {currentNode.label} ({currentNode.subType})
      </p>
    </div>
  )
}
...
export default function ExecutionDetailPage() {
  ...
  // Find the current node (the one that's running or the last executed)
  const currentNode = useMemo((): AutomationNode | null => {
    ...
    // Otherwise, find the last executed step
  ...
  // Determine if execution is in waiting state
  const isWaiting = useMemo(() => {
    if (!execution?.steps || execution.status !== "Running") return false
    
    // Check if any step has wait output and is the current step
  ...
  // Get display status accounting for waiting state
    ...
    return (
      <div>
        <PageHeader title="Execution Details">
          <Link href={`/events/${eventId}/automations/${automationId}/logs`}>
  ...
  return (
    <div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        {/* Left column: Execution info and steps */}
        <div className="space-y-6">
          ...
          <Card>
            <CardHeader>
              <div className="flex items-center justify-between">
                <CardTitle className="flex items-center gap-2">
                  <Zap className="h-5 w-5 text-ora-gold" />
                  Execution Summary
                </CardTitle>
            ...
            <CardContent className="space-y-4">
              ...
              {/* Trigger.dev Run ID - Requirement 10.1 */}
              ...
              {/* Current Node - Requirement 10.4 - temporarily disabled due to type issue */}

              {/* Error Display - Requirement 10.5 */}
              {execution.error ? (
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <Play className="h-5 w-5 text-ora-gold" />
                Execution Steps
              </CardTitle>
              <CardDescription>
                  ...
                  {execution.steps.map((step, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {/* Wait/Delay Information - Requirement 10.3 */}
                        {waitInfo && (
